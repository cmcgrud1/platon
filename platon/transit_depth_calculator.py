       ...
       '''
        ...
        if InstNum:
            for I in range(InstNum): #CHIMA. To add offsets
                cond1 = binned_wavelengths > Offset_kwargs["Offset"+str(I+1)+"_Wavs"][0]
                cond2 = binned_wavelengths < Offset_kwargs["Offset"+str(I+1)+"_Wavs"][1]
                binned_depths[np.logical_and(cond1, cond2)] += Offset_kwargs["Offset"+str(I+1)]
        if full_output:
